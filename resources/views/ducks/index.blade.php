...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Duck Status</div>
                ...
                <div class="card-body">
                    ...
                    @if ( empty($duck->name) )
                        You do not currently have a duck. Click <a href="{{ url('/ducks/create') }}">here</a> to get your first duck.
                    @else
                        @if ( $duck->status == "Deceased" ) 
                            <h3>RIP {{ $duck->name }} you will be missed dearly.</h3>
                        @else
                            <h3>Here is your duck! Say hello to {{ $duck->name }}.</h3>
                        @endif
                        
                        <input name="id" id="id" type="hidden" value="{{ $duck->id }}" class="form-control">
                        <?xml version="1.0" encoding="utf-8"?>
                        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             viewBox="0 0 595.28 595.28" style="enable-background:new 0 0 595.28 595.28;width:300px;height:300px;@if ( $duck->status !== "Deceased" ) fill:#facf58; @endif" xml:space="preserve">
                        <path  d="M517.88,199.22c-0.11-0.24-0.24-0.48-0.39-0.71c-3.02-4.62-12.55-4.11-17.27-5.03
                        ...
                        <div class="col-md-5 float-right">
                            <div class="card">
                                <div class="card-header">{{ $duck->name }} is {{ $duck->status }}</div>
                                <div class="card-body">
                                    @if ( $duck->status == "Deceased" ) 
                                        <p>{{ $duck->name }} has passed away. I am very sorry for your loss.</p>
                                        <p>If you would like to try again with another Duck. You may do so <a href="{{ url('/ducks/create') }}">here</a>.</p>
                                    @else
                                        <p>Born on {{ \Carbon\Carbon::parse($duck->born)->format('Y-m-d') }} at {{ \Carbon\Carbon::parse($duck->born)->format('H:i:s') }}</p>
                                        <p>Health: {{ $duck->health }}</p>
                                        <p>Hunger: {{ $duck->hunger }}</p>
                                        <p>Weight: {{ $duck->weight }}lb</p>
                                        <p>Sex: {{ $duck->sex }}</p>
                                        <p>Happyness: {{ $duck->happyness }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        @if ( $duck->status !== "Deceased" )
                            <div class="col-md-12">
                                <button type="button" class="btn btn-default duck-action" data-duck-action="feed">Feed {{ $duck->name }}</button>
                                <button type="button" class="btn btn-default duck-action" data-duck-action="play">Play with {{ $duck->name }}</button>
                                @if ( $duck->illness )
                                    <button type="button" class="btn btn-default duck-action" data-duck-action="heal">Heal {{ $duck->name }}</button>
                                @endif
                            </div>
                        @endif
                    @endif
...
$(document).ready(function(){
    ...
    $(".duck-action").click(function(e){
        e.preventDefault();
        var id = $("#id").val();
        var duck_action = $(this).data('duckAction');

        if(typeof id !== 'undefined' && id > 0) {
            $(".alert-danger").addClass("d-none");
            $.ajax({
                type:'POST',
                url: '{!! url('/') !!}/ducks/'+id+'/'+duck_action,
                data: {id: id},
                success:function(data){
                    if(data.success) return window.location = '{!! url('/') !!}/ducks';

                    if(typeof data.message !== 'undefined') {
                        $(".alert-danger").text(data.message).removeClass("d-none");
                    }
               }
            });
        }
